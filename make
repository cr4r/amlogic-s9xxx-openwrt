...
EOF
    ...
    #patch wifi
    netifd_file=${configfiles_path}/patches/netifd/wireless
    ...
    wifi_file=${configfiles_path}/patches/wifi
    if [ -d "${wifi_file}" ];then
...
make_image() {
    cd ${make_path}
    build_op=${1}
    build_image_file="${out_path}/cr4rWRT_${build_op}_k${kernel}_$(date +"%Y.%m.%d.%H%M").img"
